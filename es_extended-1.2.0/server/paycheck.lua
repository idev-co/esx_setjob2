ESX.StartPayCheck = function()
    function payCheck()
        ...
        for i=1, #xPlayers, 1 do
            ...
            local salary2  = xPlayer.job.grade_salary
            ...
            elseif (Config.PaycheckJob2) then
                if job2 == 'unemployed2' then -- unemployed
                    xPlayer.addAccountMoney('bank', salary2)
                    TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck2'), _U('received_help', salary2), 'CHAR_BANK_MAZE', 9)
                elseif Config.EnableSocietyPayoutsJob2 then -- possibly a society
                    TriggerEvent('esx_society:getSociety', xPlayer.job2.name, function (society)
                        if society ~= nil then -- verified society
                            ...
                            xPlayer.addAccountMoney('bank', salary2)
                            TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, _U('bank'), _U('received_paycheck2'), _U('received_salary2', salary), 'CHAR_BANK_MAZE', 9)
                        end
                    end)
                else -- generic job
                    xPlayer.addAccountMoney('bank', salary2)
