...
ESX.RegisterCommand('setjob2', 'admin', function(xPlayer, args, showError)
	if ESX.DoesJob2Exist(args.job, args.grade) then
		...
		showError('That job2 does not exist')
	end
end, true, {help = _U('setjob2'), validate = true, arguments = {
	{name = 'playerId', help = _U('id_param'), type = 'player'},
	{name = 'job2', help = _U('setjob_param2'), type = 'string'},
	{name = 'grade2', help = _U('setjob_param3'), type = 'number'}
}})
